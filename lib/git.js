...
exports.GitStorage = function (repo, ref) {
  ...
  var refs = this._repo.readRefs();
  var sha;
  for (var i = 0; i < prefixes.length; ++i) {
    var prefix = prefixes[i];
    if (refs.hasOwnProperty(prefix + ref)) {
      var link = refs[prefix + ref];
      while (link.substr(0, 5) == 'ref: ')
        link = refs[link.substr(5)];
      sha = link;
      break;
    }
  }
  ...
  var file = this._repo.catFile(sha);
  if (file.type == 'tag')
    file = this._repo.catFile(file.data.range(7, 47));
  if (file.type != 'commit')
...
exports.GitStorage.prototype._walk = function (sha) {
  var result = {};
  var data = this._repo.catFile(sha).data;
  do {
...
exports.GitStorage.prototype.read = function (path) {
  ...
  if (!entry)
    throw fs.NoSuchEntryError('No such entry');
  if (!(entry instanceof Binary))
    throw fs.EntryIsDirError('Entry is dir');
  return this._repo.catFile(entry).data;
};
