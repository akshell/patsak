...
exports.GitStorage = function (repo, ref) {
  ...
  for (var i = 0; i < prefixes.length; ++i) {
    ...
    if (this._repo.refs.hasOwnProperty(prefix + ref)) {
      var link = this._repo.refs[prefix + ref];
      while (link.substr(0, 5) == 'ref: ')
        link = this._repo.refs[link.substr(5)];
      sha = link;
      break;
    }
  }
  ...
  var file = this._repo.read(sha);
  if (file.type == 'tag')
    file = this._repo.read(file.data.range(7, 47));
  if (file.type != 'commit')
...
exports.GitStorage.prototype._walk = function (sha) {
  var result = {};
  var data = this._repo.read(sha).data;
  do {
...
exports.GitStorage.prototype.read = function (path) {
  ...
  return this._repo.read(entry).data;
};
